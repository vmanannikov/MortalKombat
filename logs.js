...
function getTextLog (type, playerName1, playerName2) {
    switch (type) {
        case 'start':
            ...
            break;
        case 'end':
            return logs[type][randNum(logs[type].length - 1) - 1]
...
function generateLogs (type, player1 = {}, player2 = {}, valueAttack) {
    let log = getTextLog(type, player1.name, player2.name)

    switch (type) {
        case 'hit':
            log = `${getDateTime()} ${log} -${valueAttack} [${player2.hp}/100]`;
            break;
        case 'defence':
        case 'end':
        case 'draw':
...
export { logs, getTextLog, generateLogs };